export async function POST(request) {
  try {
    ...
    if (session.error) {
      return Response.json({ error: session.error.message, code: session.error.code, type: session.error.type }, { status: 500 })
    }

    if (!session.url) {
      return Response.json({ error: 'No URL returned', session: JSON.stringify(session) }, { status: 500 })
    }
